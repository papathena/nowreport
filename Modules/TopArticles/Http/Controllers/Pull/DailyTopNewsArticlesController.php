...
class DailyTopNewsArticlesController extends Controller
{
	/* B: Store Daily Data */
    protected function dailyStore(
        ...
        String $title)
	{
		$gatoparticles = new DailyTopNewsArticles();
    ...
    protected function dailyUpdate(
        ...
        String $title)
    {
        $gavisitors = DailyTopNewsArticles::where([
            'date' => $date,
            'devices' => $devices,
            'channels' => $channels,
            'pageTitle' => $title,
    ...
    {
        $datas = DailyTopNewsArticles::where([
            'date' => $dates,
            'devices' => $devices,
            'channels' => $channels,
            'pagetitle' => $title,
    ...
    public function dailyGetData(Period $period, String $platform, String $devices, String $channels=NULL, String $filters=NULL)
    {
        ...
        switch ($platform) {
            case 'android' : $data = $getQuery->androidTopNewsArticles($period, $devices, $channels, $filters);
                break;
            case 'ios' : $data = $getQuery->iosTopNewsArticles($period, $devices, $channels, $filters);
                break;
            default : $data = $getQuery->webTopNewsArticles($period, $devices, $channels, $filters);
        }
        return $data;
    }
    /* E: Get Daily Data */
                            ...
                            } elseif ($this->dailyLessData($dataArticle['date'],$dev,$channel, $dataArticle['visitors'],$dataArticle['pageviews'],$dataArticle['pageTitle']))
 {
                    ...
                    $filters = config('top10articles.topTextArticless_filter.'.$filter_idx);
