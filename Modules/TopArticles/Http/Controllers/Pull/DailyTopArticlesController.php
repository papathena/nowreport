...
class DailyTopArticlesController extends Controller
{
    ...
    protected function dailyStore(
        ...
        String $title)
	{
		$gatoparticles = new DailyTopArticles();
    ...
    protected function dailyUpdate(
        ...
        String $title)
    {
        $gavisitors = DailyTopArticles::where([
            'date' => $date,
            'devices' => $devices,
            'channels' => $channels,
            'pageTitle' => $title,
    ...
    public function dailyGetData(Period $period, String $platform, String $devices, String $channels=NULL, String $filters=NULL)
    {
        ...
        switch ($platform) {
            case 'android' : $data = $getQuery->androidTopArticles($period, $devices, $channels, $filters);
                break;
            case 'ios' : $data = $getQuery->iosTopArticles($period, $devices, $channels, $filters);
                break;
            default : $data = $getQuery->webTopArticles($period, $devices, $channels, $filters);
        }
        return $data;
    }
    /* E: Get Daily Data */
