...
use Modules\TopArticles\Helpers\TopPhotoArticlesHelper;
use Carbon\Carbon;
use Spatie\Analytics\Period;

class MonthlyTopPhotoArticlesController extends Controller
{
    ...
    protected function monthlyStore(
        ...
        String $title)
    {
        $gatoparticles = new MonthlyTopPhotoArticles();
    ...
    protected function MonthlyUpdate(
        ...
        String $title)
    {
        $gatoparticles = MonthlyTopPhotoArticles::where([
            'monthYear' => $month,
            'year' => $year,
            'devices' => $devices,
            'channels' => $channels,
    ...
    {
        $datas = MonthlyTopPhotoArticles::where([
            'monthYear' => $month,
            'devices' => $devices,
            'channels' => $channels,
            'pagetitle' => $title,
    ...
    {
        $getQuery = new TopPhotoArticlesHelper();
        switch ($platform) {
            case 'android' : $data = $getQuery->androidTopPhotoArticlesMonthly($period, $devices, $channels, $filters);
                break;
            case 'ios' : $data = $getQuery->iosTopPhotoArticlesMonthly($period, $devices, $channels, $filters);
                break;
            default : $data = $getQuery->webTopPhotoArticlesMonthly($period, $devices, $channels, $filters);
        }
        return $data;
    }
    /* E: Get Monthly Data*/
    ...
    public function monthlyPull(Request $request)
    {
        if ($request->isMethod('get')) {
            ...
            for ($a=0; $a<=$lenMonths; $a++) {
                ...
                if ($platform == 'web') {
                    $filters = config('top10articles.topPhotoArticles_filter.'.$channel);
                    ...
                    $filters = config('top10articles.topPhotoArticles_filter.'.$filter_idx);
