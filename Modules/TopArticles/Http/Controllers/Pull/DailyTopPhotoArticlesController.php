...
class DailyTopPhotoArticlesController extends Controller
{
	/* B: Store Daily Data */
    protected function dailyStore(
        ...
        String $title)
	{
		$gatoparticles = new DailyTopPhotoArticles();
    ...
    protected function dailyUpdate(
        ...
        String $title)
    {
        $gavisitors = DailyTopPhotoArticles::where([
            'date' => $date,
            'devices' => $devices,
            'channels' => $channels,
            'pageTitle' => $title,
    ...
    {
        $datas = DailyTopPhotoArticles::where([
            'date' => $dates,
            'devices' => $devices,
            'channels' => $channels,
            'pagetitle' => $title,
    ...
    public function dailyGetData(Period $period, String $platform, String $devices, String $channels=NULL, String $filters=NULL)
    {
        ...
        switch ($platform) {
            case 'android' : $data = $getQuery->androidTopPhotoArticles($period, $devices, $channels, $filters);
                break;
            case 'ios' : $data = $getQuery->iosTopPhotoArticles($period, $devices, $channels, $filters);
                break;
            default : $data = $getQuery->webTopPhotoArticles($period, $devices, $channels, $filters);
        }
        return $data;
    }
    /* E: Get Daily Data */
                            ...
                            } elseif ($this->dailyLessData($dataArticle['date'],$dev,$channel, $dataArticle['visitors'],$dataArticle['pageviews'],$dataArticle['pageTitle']))
 {
                    ...
                    $filters = config('top10articles.topPhotoArticless_filter.'.$filter_idx);
