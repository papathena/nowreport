...
use Modules\TopArticles\Helpers\TopArticlesHelper;
use Carbon\Carbon;
...
class MonthlyTopArticlesController extends Controller
{
    ...
    protected function monthlyStore(
        ...
        String $title)
    {
        $gatoparticles = new MonthlyTopArticles();
    ...
    protected function MonthlyUpdate(
        ...
        String $title)
    {
        $gatoparticles = MonthlyTopArticles::where([
            'monthYear' => $month,
            'year' => $year,
            'devices' => $devices,
            'channels' => $channels,
    ...
    {
        $datas = MonthlyTopArticles::where([
            'monthYear' => $month,
            'devices' => $devices,
            'channels' => $channels,
            'pagetitle' => $title,
    ...
    {
        $getQuery = new TopArticlesHelper();
        switch ($platform) {
            case 'android' : $data = $getQuery->androidTopArticlesMonthly($period, $devices, $channels, $filters);
                break;
            case 'ios' : $data = $getQuery->iosTopArticlesMonthly($period, $devices, $channels, $filters);
                break;
            default : $data = $getQuery->webTopArticlesMonthly($period, $devices, $channels, $filters);
        }
        return $data;
    }
    /* E: Get Monthly Data*/
    ...
    public function monthlyPull(Request $request)
    {
        if ($request->isMethod('get')) {
            ...
            for ($a=0; $a<=$lenMonths; $a++) {
                ...
                if ($platform == 'web') {
                    $filters = config('top10articles.toparticles_filter.'.$channel);
                    ...
                    $filters = config('top10articles.toparticles_filter.'.$filter_idx);
